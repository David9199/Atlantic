    ...
    if (window.ethereum) { // 检查浏览器是否安装了支持Ethereum的插件，如MetaMask
        ...
        const connectWallet = async () => {
            try {
                ...
                const roundedBalance = parseFloat(ethers.formatUnits(balance)).toFixed(4);
        ...
        return (
            // text-lg font-bold
            <button className="text-white hover:bg-blue-700" onClick={connectWallet}>
                {walletInfo.address ? `Address1: ${walletInfo.address} - Balance: ${walletInfo.balance} ETH` : 'Connect Wallet'}
            </button>
